...
document.getElementById("checkInForm").addEventListener("submit", async function (event) {
    ...
    const roomNumber = Math.floor(Math.random() * 100) + 1; // Random number between 1 and 100
...
async function appendData(guestName, roomNumber) {
    ...
    try {
        ...
        document.getElementById("message").textContent = `Check-in request submitted successfully! Assigned Room: ${roomNumber}`;
